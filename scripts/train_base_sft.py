#!/usr/bin/env python3
"""
QLoRA Fine-tuning for Standard Llama Base Models
Supports the same mixed dataset configuration as MoE training
"""
...
class ModelArguments:
    model_name_or_path: str = field(default="models/Llama-3.2-3B")
    use_flash_attn: bool = field(default=True, metadata={"help": "Use Flash Attention 2"})
    use_qlora: bool = field(default=True, metadata={"help": "Use QLoRA (4-bit + LoRA)"})
...
def train():
    ...
    if "wandb" in training_args.report_to:
        wandb.init(project="Llama-Base-SFT", name=training_args.run_name)
    
    # Load data configuration
    logger.info(f"\n{'='*80}")
    logger.info("QLoRA Fine-tuning for Standard Llama Base Model")
    logger.info(f"{'='*80}\n")
