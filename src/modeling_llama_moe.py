...
class LlamaMoERotaryEmbedding(nn.Module):
...
# Copied from transformers.models.llama.modeling_llama.LlamaLinearScalingRotaryEmbedding with Llama->LlamaMoE
...
# Copied from transformers.models.llama.modeling_llama.LlamaDynamicNTKScalingRotaryEmbedding with Llama->LlamaMoE
...
class LlamaMoEMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.experts = nn.ModuleList([LlamaMoEMLP(config, intermediate_size = config.moe_intermediate_size) for i in range(config.n_routed_experts)])
        self.gate = MoEGate(config)
        if config.n_shared_experts is not None:
            intermediate_size = config.moe_intermediate_size * config.n_shared_experts
            self.shared_experts = LlamaMoEMLP(config=config, intermediate_size = intermediate_size)
...
# Copied from transformers.models.llama.modeling_llama.LlamaAttention with Llama->LlamaMoE
class LlamaMoEAttention(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            # Support both standard format (type, factor) and Llama 3 format (rope_type, factor, ...)
            rope_type = self.config.rope_scaling.get("rope_type", None)
            if rope_type is not None:
                # Llama 3 format - use dynamic scaling for llama3 type
                scaling_factor = self.config.rope_scaling.get("factor", 1.0)
                # For llama3 type, we use dynamic NTK scaling
                ...
                # Standard format
                scaling_type = self.config.rope_scaling.get("type", None)
                scaling_factor = self.config.rope_scaling.get("factor", None)
                if scaling_type is None or scaling_factor is None:
                    raise ValueError(
                        f"`rope_scaling` must have 'type' and 'factor' fields, got {self.config.rope_scaling}"
                    )
...
# Copied from transformers.models.llama.modeling_llama.LlamaFlashAttention2 with Llama->LlamaMoE
class LlamaMoEFlashAttention2(LlamaMoEAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # LlamaMoEFlashAttention2 attention does not support output_attentions
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be removed in v4.37. Please make sure use `attention_mask` instead.`"
            )
        ...
        # in fp32. (LlamaMoERMSNorm handles it correctly)
    ...
    def _flash_attention_forward(
        ...
        if not self._flash_attn_uses_top_left_mask:
            ...
            # TODO: Remove the `query_length != 1` check once Flash Attention for RoCm is bumped to 2.1. For details, please see the comment in LlamaMoEFlashAttention2 __init__.
...
# Copied from transformers.models.llama.modeling_llama.LlamaSdpaAttention with Llama->LlamaMoE
class LlamaMoESdpaAttention(LlamaMoEAttention):
    ...
    # Adapted from LlamaMoEAttention.forward
    def forward(
        self,
        hidden_states: torch.Tensor,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "LlamaMoEModel is using LlamaMoESdpaAttention, but `torch.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
...
class LlamaMoEDecoderLayer(nn.Module):
    def __init__(self, config: LlamaMoEConfig, layer_idx: int):
        ...
        self.self_attn = LlamaMoE_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)

        self.mlp = LlamaMoEMoE(config) if (config.n_routed_experts is not None and  \
                                           layer_idx >= config.first_k_dense_replace and layer_idx % config.moe_layer_freq == 0) \
                                        else LlamaMoEMLP(config)
...
@add_start_docstrings(
    "The bare LlamaMoE Model outputting raw hidden-states without any specific head on top.",
    LlamaMoE_START_DOCSTRING,
)
...
class LlamaMoEModel(LlamaMoEPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(LlamaMoE_INPUTS_DOCSTRING)
    def forward(
        self,
...
class LlamaMoEForCausalLM(LlamaMoEPreTrainedModel):
    ...
    def forward(
        ...
        >>> model = LlamaMoEForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
@add_start_docstrings(
    """
    The LlamaMoE Model transformer with a sequence classification head on top (linear layer).
